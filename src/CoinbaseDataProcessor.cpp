...
void CoinbaseDataProcessor::parse_and_publish(const std::string& message) {
    ...
    const char* data_start = CoinbaseFastParser::find_value_after_key(start, end, "data", 4);
    if (!data_start) return; // "data" key not found

    // 2. Check the event type "e"
    const char* event_type_val = CoinbaseFastParser::find_value_after_key(data_start, end, "e", 1);
    ...
    if (strncmp(event_type_val, "trade", 5) == 0) {
        ...
        // Extract symbol "s"
        const char* symbol_val = CoinbaseFastParser::find_value_after_key(data_start, end, "s", 1);
        if (symbol_val) {
        ...
        const char* price_val = CoinbaseFastParser::find_value_after_key(data_start, end, "p", 1);
        if (price_val) {
        ...
        const char* quantity_val = CoinbaseFastParser::find_value_after_key(data_start, end, "q", 1);
        if (quantity_val) {
        ...
        const char* time_val = CoinbaseFastParser::find_value_after_key(data_start, end, "T", 1);
        if (time_val) {
