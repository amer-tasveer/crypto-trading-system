...
void KrakenDataProcessor::parse_and_publish(const std::string& message) {
    ...
    // const char* event_type_val = KrakenFastParser::find_value_after_key(data_start, end, "e", 1);
    ...
    //     trade_data.source = "Kraken";

    //     // Extract symbol "s"
    //     const char* symbol_val = KrakenFastParser::find_value_after_key(data_start, end, "s", 1);
    //     if (symbol_val) {
    ...
    //     const char* price_val = KrakenFastParser::find_value_after_key(data_start, end, "p", 1);
    //     if (price_val) {
    //         // The parser will stop at the closing quote
    //         trade_data.price = KrakenFastParser::parse_double(price_val, end);
    //     }

    //     // Extract and parse quantity "q"
    //     const char* quantity_val = KrakenFastParser::find_value_after_key(data_start, end, "q", 1);
    //     if (quantity_val) {
    //         trade_data.quantity = KrakenFastParser::parse_double(quantity_val, end);
    //     }

    //     // Extract and parse trade time "T" (this is a raw number, not a string)
    //     const char* time_val = KrakenFastParser::find_value_after_key(data_start, end, "T", 1);
    //     if (time_val) {
    ...
    //     order_book_event.data = KrakenFastParser::parse_depth_update(message.c_str(), strlen(message.c_str()));
